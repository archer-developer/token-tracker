...
import type { Instrument, PurchaseLot } from '@/db/types'
...
function makeLot(
  ...
  return {
    ...
    purchaseDate,
    quantity: 1,
    pricePerToken: totalCost,
    totalCost,
    createdAt: purchaseDate + 'T00:00:00.000Z',
  }
}
...
// Single lot purchased on startDate — no ownership gaps, same behaviour as fixed principal
const baseLots = [makeLot(1, 1, '2024-01-01', PRINCIPAL)]

describe('generateSchedule — structure', () => {
  it('generates 12 coupon records + 1 redemption for a 1-year monthly bond', () => {
    const records = generateSchedule(base, baseLots)
  ...
  it('generates 4 coupon records for a 1-year quarterly bond', () => {
    const records = generateSchedule({ ...base, paymentFrequency: 'quarterly' }, baseLots)
  ...
  it('returns empty array when instrument has no id', () => {
    ...
    expect(generateSchedule(noId as Instrument, baseLots)).toHaveLength(0)
  })
  ...
  it('generates zero-amount coupon records when there are no lots', () => {
    const records = generateSchedule(base, [])
...
// Single lot on startDate — useful for testing the period structure and formula
// without ownership gaps
const gurminaLotsOnStart = [makeLot(1, 1, '2024-12-30', 10040)]

// Real purchase lots: Lot 1 on Jan 9, Lot 2 on Jan 26 (see gurnima-algorithm.md)
const gurminaRealLots: PurchaseLot[] = [
  makeLot(1, 1, '2025-01-09', 5680),
  makeLot(2, 1, '2025-01-26', 4360),
]

describe('generateSchedule — GURMINA.USD.2024.01 real payments', () => {
  it('generates 25 coupon periods + 1 redemption', () => {
    const records = generateSchedule(gurmina, gurminaLotsOnStart)
  ...
  it('15.02.2025 — January 2025: weighted by lot purchase dates = 47.25 USD', () => {
    // Lot 1 (Jan 9–25, 17 days): 5680 × 0.11 × 17 / 365 = 29.10...
    // Lot 2 (Jan 26–31,  6 days): 10040 × 0.11 × 6 / 365 = 18.15...
    // Total: 47.254... → 47.25
    const records = generateSchedule(gurmina, gurminaRealLots)
  ...
  it('15.03.2026 — February 2026 (28 days, 365-day year) = 84.72 USD', () => {
    // 10040 × 0.11 × 28 / 365 = 84.721... → 84.72
    const records = generateSchedule(gurmina, gurminaLotsOnStart)
  ...
  it('15.04.2026 — March 2026 (31 days, 365-day year) = 93.80 USD', () => {
    // 10040 × 0.11 × 31 / 365 = 93.798... → 93.80
    const records = generateSchedule(gurmina, gurminaLotsOnStart)
  ...
  it('15.05.2026 — April 2026 (30 days, 365-day year) = 90.77 USD', () => {
    // 10040 × 0.11 × 30 / 365 = 90.772... → 90.77
    const records = generateSchedule(gurmina, gurminaLotsOnStart)
